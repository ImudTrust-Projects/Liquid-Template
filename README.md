...
2. If necessary, update your `<GamePath>` (your Gorilla Tag directory) in `Directory.Build.props`.
3. Change the menu name in `PluginInfo.cs`.
4. Customize the menu appearance in `Menu/Settings.cs`.
5. Add or modify buttons in `Menu/Buttons.cs`.
6. Build the project using `Ctrl + Shift + B`.

After building the project, the compiled file will automatically be placed in your `plugins` folder.